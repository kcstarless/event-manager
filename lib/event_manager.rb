...
def clean_phone_number(phone)
  ...
  if phone.size == 10
    ...
    phone = phone.size == 11 && phone[0] == '1' ? phone[1..11] : 'xxxxxxxxxx'
  end
  phone.insert(3, '-').insert(-5, '-')
end

def peak_reg_hours(reg_hour_count)
  peak_hour = reg_hour_count.select { |_index, value| value == reg_hour_count.values.max }.keys
  "Peak registeration hours: #{peak_hour.join(', ')} hours"
end

def peak_reg_days(reg_day_count)
  peak_day = reg_day_count.select { |_index, value| value == reg_day_count.values.max }.keys.join('')
  "Peak registeration day of week: #{Date::DAYNAMES[peak_day.to_i]}"
end
